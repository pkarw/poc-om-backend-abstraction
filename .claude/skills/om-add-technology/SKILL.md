...
- **Full module-contract surface is mandatory** (`specs/10-module-contract-parity.md`): the module abstraction MUST let every module declare the same four pieces upstream declares — **RBAC feature declarations** (`acl.ts`), **notification types** (`notifications.ts`), **custom-field sets / custom entities** (`ce.ts` + `data/fields.ts`), and **declared typed events** (`events.ts`) — the *same way* in this tech as in every other package. The registry/composition layer MUST aggregate these across enabled modules (a package-wide list of features, notification types, field sets, and events). Declarations are first-class even when the engine that acts on them is deferred (declare-now); the abstraction carries the declaration regardless.
...
- **Subagent B — app skeleton**: the runtime source tree per spec 09 — a **module abstraction / contract** that exposes the full declaration surface per `specs/10` (id, routes, entities/migrations, DI, plus **feature declarations, notification types, custom-field sets/custom entities, and declared typed events**), a **module registry/composition config that aggregates those declarations across enabled modules** (package-wide features / notification-type / field-set / event registries), a dispatch pipeline stub honoring `specs/01`/`02` guard order and default-deny auth, queue abstraction with `QUEUE_STRATEGY` selection (`async` ⇒ BullMQ/Redis, else local) and the `{id,payload,createdAt}` envelope, worker host entrypoint, migration tool wiring, `/healthz` endpoint (200 JSON, no auth), and one **example `health_check` module** exercising every convention slot including the declaration surface: one public GET route with an openApi-equivalent description, one entity + migration, one event + subscriber, one worker + queue, **one ACL feature declaration, one notification-type declaration, one custom-field set declaration, and one declared typed event** (its delivery/EAV engine may be a stub, but the declaration must aggregate into the registry), DI registration, and tests for each — including a test asserting the four declarations appear in the aggregated registry.
- **Subagent C — docs**: `AGENTS.md` following the REQUIRED section outline from spec 09 (it must map every upstream concept — module layout, routes, entities/migrations, validators, subscribers, workers, ACL/**feature declarations**, **notification types**, **custom-field sets / custom entities**, **declared typed events**, DI, setup, CLI — to this tech's idiom, and document the **module-contract declaration mechanism + registry aggregation** per `specs/10` so om-port-module knows exactly how a ported module declares each piece; om-port-module reads this as law); `README.md` per the standard structure (native + Docker quickstart, port, make targets); ADRs in `docs/decisions/`: `0001-runtime-and-framework.md`, `0002-orm-and-migrations.md`, `0003-queue-bullmq-compatibility.md` (each `Status / Context / Decision / Consequences`; the queue ADR states the honest interop level).
...
3. `make migrate` on the fresh DB (applies the health_check migration), `make test` green — including the test that the health_check module's four declarations (feature, notification type, custom-field set, declared event) surface in the package's aggregated registry (`specs/10`).
