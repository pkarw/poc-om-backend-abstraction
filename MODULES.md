...
> **customers (.NET):** all 4 phases ported (25 tables, ~90 routes, 51 events, 5 custom-field sets) via the command bus + CRUD factory + entities(custom fields) + query_index(cf indexation); 223 tests; live-verified standalone (list/detail/create/deals/cf-filter). **Testbench integration ✅:** seeding is now 100% on the .NET side — each module owns its setup (`IModule.SeedDefaults/SeedExamples`, the port of upstream `setup.ts`), run by `ModuleSeedRunner` on boot; OM is **migrate-only** (`mercato init` no longer runs). Against a **real OM** on a shared DB the .NET-seeded CRM reads back cleanly through the proxy — 6 people / 3 companies / 6 deals + default pipeline, content readable (no DEK-encryption gap, since the port both writes and reads). See `testbench/INTEGRATION-customers.md` + `./integration-customers.sh`.
