...
| H | Run OM's own integration tests vs .NET | TODO (LAST, after B/C/D/E) | Open Mercato ships thousands of integration tests (incl. ported modules). Run them against the .NET-served backend on the shared DB; make green, fill gaps. OM tests live under `packages/*/src/**/__integration__` / `__tests__` in the OM checkout; find the harness that points at a base URL and target the testbench (`http://localhost:8088`). |
